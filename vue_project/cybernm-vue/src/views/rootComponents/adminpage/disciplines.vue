<template>
  <v-container fluid>
    ...
    <v-row dense>
      <v-col v-for="card in disciplineList" :key="card.id" :cols="card.flex">
        <v-card>
          <v-img
            :src="card.discipline_image"
            alt="discipline"
            class="i white--text align-end"
          ...
          <v-card-actions>
            ...
            <v-icon class="mr-2" @click="editItem(card)"> mdi-pencil </v-icon>
            <v-icon @click="deleteItem(card)"> mdi-delete </v-icon>
          </v-card-actions>
        </v-card>
      </v-col>
    </v-row>
...
export default {
  ...
  methods: {
    ...
    async addDiscipline() {
      await fetch(`${this.$store.getters.getServerUrl}Adddiscipline/`, {
        method: "post",
        headers: {
          "Content-Type": "application/json",
        },

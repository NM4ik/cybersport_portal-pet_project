<template>
  <v-container class="pa-0">
    <div class="main_info">
      ...
      <div class="card">
        <v-card class="ml-15 v-cards" elevation="2">
          <v-card-title
            >tournament: <span>{{ tournament.name }}</span></v-card-title
          >
          <v-card-subtitle
            >discipline:
            <span>{{ tournament.discipline_id }}</span></v-card-subtitle
          >
          <v-card-text>
            <ul class="tournament_description">
              <li>
                status: <span>{{ tournament.status }}</span>
              </li>
              <li>
                seats <span>{{ tournament.seats_number }}</span>
              </li>
              <li>
                limitation: <span>{{ tournament.limitation }}</span>
              </li>
              <li>
                prize: <span>{{ tournament.prize }}</span>
              </li>
              <li>
                start date: <span>{{ tournament.start_date }}</span>
              </li>
              <li>
                end date: <span>{{ tournament.end_date }}</span>
              </li>
            </ul>
          </v-card-text>
          <v-card-actions>
    ...
    <v-card flat>
      <v-card-title class="data_input"> </v-card-title>
      <v-data-table :headers="headers" :items="playersList">
